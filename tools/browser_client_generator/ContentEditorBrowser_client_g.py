...
class ContentEditorBrowser_client(BaseContentEditor):
    ...
    def add_html(self, model):
        ...
        code = code.replace("$proper_name$", self.str_proper_convertor(model_name))
        code = code.replace("$proper_action$",self.str_proper_convertor(action))
        code = code.replace("$proper_element$",self.str_proper_convertor(element))
    ...
    def add_form_html(self, model):
        ...
        for i in data:
            ...
            editor.add_to_content_nl(f'<div {hidden}>{self.str_proper_heading_convertor(i["name"])}</div>')
            editor.add_to_content_nl(f"<div {hidden}>")

            editor.add_to_content_nl(f'<input type="{datatype}" id="{i["name"]}" name="{i["name"]}" {null_value}>')
            
            editor.add_to_content_nl(f'</div>')


        editor.add_to_content_nl(f'<input type="submit">')
        editor.add_to_content_nl(f'</form>')
