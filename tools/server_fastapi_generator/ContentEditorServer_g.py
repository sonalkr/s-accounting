...
class ContentEditorServer(BaseContentEditor):
    ...
    def add_dependency_FastApi_Router_BaseModel(self, router_name='default_api'):
        ...
        # self.add_tab().add_to_content_nl(
        #     'cls.__signature__ = cls.__signature__.replace(parameters=[arg.replace(default=Form(...))for arg in cls.__signature__.parameters.values()])')
        # self.add_tab().add_to_content_nl('return cls')
    ...
    def add_default_model(self, model: dict):
        ...
        self.add_to_content_nl('@as_form')
        self.add_to_content_nl(
            f'class {model["model_name"].capitalize()}(BaseModel):')
    ...
    def add_update_api(self, model, router_name='default_api'):
        self._add_api_function_head_create(
            model, router_name, function_name='update', method='put')
        ...
        self.add_tab().add_to_content_nl("query = 'UPDATE " + model_name +
                                         " SET {}'.format(', '.join(\"%s='%s'\" % (k) for k in list_of_data))")
    ...
    def add_get_api(self, model, router_name='default_api'):
        self._add_api_function_head_get(
            model, router_name, function_name='get', method='get')
        model_name = model["model_name"]
        self.add_tab()
        self.add_to_content_nl(
            f'rs = db.execute("SELECT * FROM {model_name};").all()')
        self.add_tab()
    ...
    def _add_api_function_head_create(self, model, router_name, function_name, method):
        ...
        self.add_to_content_nl(
            f'async def {function_name}_{model["model_name"]}({model["model_name"]}: {model["model_name"].capitalize()} = Depends({model["model_name"].capitalize()}.as_form), db: Session = Depends(get_db)):')
    ...
    def add_delete_api(self, model, router_name='default_api'):
        self._add_api_function_head_delete(
            model, router_name, function_name='delete', method='delete')
        model_name = model["model_name"]
        self.add_tab()
        ...
        self.add_to_content_nl(
            f'rs = db.execute(f"SELECT * FROM {model_name} WHERE id = ' + '{str(id)}' + ';").all()')
        self.add_tab().add_tab()
        ...
        self.add_tab().add_tab()
        self.add_to_content_nl(
            f'db.execute(f"DELETE FROM {model_name} WHERE id = ' + '{str(id)}' + ';")')
        self.add_tab()
        self.add_to_content_nl('db.commit()')
        self.add_tab()
        self.add_to_content_nl('return "ok"')
