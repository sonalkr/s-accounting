        ...
        for r in result:
            for t in r[0]['relationship']:
                ...
                for i, _ in enumerate(result):
                    ...
                    ref_relationship_list = DataFrame(result[i][0]['ref_relationship'])[
                        'model_name'].tolist() if len(result[i][0]['ref_relationship']) else list()
                    if t['model_name'] == result[i][0]['model_name'] and not model_name in ref_relationship_list:
                        # print('ok')
                        # print(result[i][0]['model_name'], model_name)
                        result[i][0]['ref_relationship'].append(
                            {'model_name': model_name, 'field': f})
